...
std::shared_ptr<BaseExpression> BooleanExpression::interpret(std::shared_ptr<Scope> scope) {
...
std::shared_ptr<BaseExpression> BooleanExpression::notOperator(std::shared_ptr<Scope> &parentScope) {
    return std::make_unique<BooleanExpression>(!contents);
}

std::shared_ptr<BaseExpression> BooleanExpression::orOperator(std::shared_ptr<BaseExpression> &&expression,
                                                          std::shared_ptr<Scope> &parentScope) {
    return std::make_unique<BooleanExpression>(
        dynamic_cast<BooleanExpression *>(expression.get())->contents || contents);
}

std::shared_ptr<BaseExpression> BooleanExpression::andOperator(std::shared_ptr<BaseExpression> &&expression,
                                                           std::shared_ptr<Scope> &parentScope) {
    return std::make_unique<BooleanExpression>(
        dynamic_cast<BooleanExpression *>(expression.get())->contents && contents);
}
