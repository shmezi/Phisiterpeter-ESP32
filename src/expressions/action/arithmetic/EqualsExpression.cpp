...
std::shared_ptr<BaseExpression> EqualsExpression::interpret(std::shared_ptr<Scope> scope) {
    return (right->interpret(scope)->equal(left->interpret(scope), scope));
}

EqualsExpression::EqualsExpression(std::unique_ptr<BaseExpression> right, std::unique_ptr<BaseExpression> left) {
